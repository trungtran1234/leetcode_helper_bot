...
@bot.slash_command(name = "leetcode", description = "Generates a random Blind 75 problem for you to solve")
async def leetcode_command(ctx):
    await ctx.respond("Generating LeetCode problem...") #Acknowledge command immediately
...
@bot.slash_command(name='approach', description = "Give your approach to the problem")
async def approach_command(ctx, *, approach):
    await ctx.respond("Generating feedback for:\n" + approach) #Acknowledge command immediately
...
@bot.slash_command(name='helpme', description = "Generates you an optimal approach to the problem")
async def help_command(ctx):
    await ctx.respond("Generating solution...") #Acknowledge command immediately
    global random_problem_title
